...
class SpatialFiLM(nn.Module):
    """
    Spatial Conditional Feature Enhancement (CFE).
    Generates per-channel gamma/beta from code and applies a lightweight
    spatial gate so modulation varies over HxW (great for composite degradations).
    """
    def __init__(self, in_dim, c_out):
        super().__init__()
        self.vec_to_gb = nn.Sequential(
            nn.Linear(in_dim, 2*c_out),
            nn.GELU(),
            nn.Linear(2*c_out, 2*c_out)
        )
        self.feat_gate = nn.Conv2d(c_out, c_out, 1)
        self.norm = LayerNorm2d(c_out)
        self.c_out = c_out

    def forward(self, feat, code):
        B, C, H, W = feat.shape
        gb = self.vec_to_gb(code)       # (B, 2C)
        gamma, beta = gb[:, :C], gb[:, C:]
        gamma = gamma[:, :, None, None]
        beta  = beta[:,  :, None, None]
        gate = torch.sigmoid(self.feat_gate(self.norm(feat)))  # (B,C,H,W)
        return (1.0 + gate) * (gamma * feat) + beta
