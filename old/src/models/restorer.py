...
from .blocks import ConvBNAct, EncoderStage, DecoderStage, FiLM, BottleneckCrossAttention
from .wavelet import HaarDWT, HaarIDWT
from .refiner import TinyDiffusionRefiner
...
class Restorer(nn.Module):
    def __init__(self, cfg,
        ...
        self.dsm = DegradationSemanticModulator(
            d_img=clip_img_dim, d_txt=clip_txt_dim, mem_txt_embed=mem_txt_embed,
            n_stages_channels=[C0, C0*2, C0*4], temperature=cfg["model"]["clip"]["temperature"]
        )
        ...
        self.hf_path = nn.Sequential(
            nn.Conv2d(3*3, C0, 3,1,1),  # pack Hh,Hv,Hd across 3 channels each
            nn.GELU(),
